...
<?php
	if($replies != null)
	{
		...
		{
			echo 'new Reply('.$reply->getId().', '. $replyposter[$reply->getId()]->getId().', "'.$replyposter[$reply->getId()]->getUsername().'", "'.$reply->getMessage().'", "'.$reply->getPostedon()->format('d.m.Y.').'", '.($op->getId()===$replyposter[$reply->getId()]->getId()?'true':'false').')';
			$count++; 
